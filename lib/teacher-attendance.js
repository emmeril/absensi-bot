...
const EMPTY_TEACHERS = { number: "", teachers: {}, schedules: {}, holidays: [] };
...
function validateSchedule(body, config) {
  const s = { id: body.id || crypto.randomUUID(), number: number(body.number), day: Number(body.day),
    subject: text(body.subject), className: text(body.className), start: body.start, end: body.end,
...
function createTeacherAttendance(deps) {
  ...
  const config = () => loadJSON(TEACHERS_PATH, EMPTY_TEACHERS);
  ...
  function getSession(token) {
    ...
    if (!session || session.expiresAt <= Date.now()) throw Object.assign(new Error("Tautan sudah berakhir. Kirim !masuk ke bot TU untuk membuka sesi yang tersedia."), { status: 410 });
    const c = config();
  ...
  async function command({ botKey, sender, body, reply }) {
    ...
    if (botKey !== `tu:${c.number}` || !c.teachers[num] || c.teachers[num].active === false) {
      await reply("Nomor ini adalah bot TU. Nomor Anda belum terdaftar sebagai guru aktif. Hubungi admin sekolah."); return true;
    }
  ...
  function report(date) {
    ...
    const c = config();
    const saved = Object.values(records().records).filter((r) => r.date === date);
    const rows = new Map(saved.map((r) => [r.key, publicRecord(r)]));
    for (const s of scheduledSessions(c, date)) {
      const key = keyFor(date, s.id);
      if (!rows.has(key)) rows.set(key, { key, date, number: s.number, name: c.teachers[s.number].name, schedule: s, hasEvidence: false });
    }
  ...
  function registerAdmin(app) {
    ...
    app.post("/api/teachers/settings", wrap(async (req, res) => {
      ...
      if (!Array.isArray(holidays) || holidays.length > 366 || holidays.some((date) => !validDate(date))) throw new Error("Daftar tanggal libur tidak valid.");
      await updateJSON(TEACHERS_PATH, (draft) => {
        if (Object.values(getClasses()).some((c) => c.waliKelas === `${num}@c.us`) || draft[TEACHERS_PATH].teachers[num] || getStudents()[`${num}@c.us`]) throw new Error("Gunakan nomor TU khusus, berbeda dari nomor siswa, guru, dan bot wali kelas.");
        Object.assign(draft[TEACHERS_PATH], { number: num, holidays: [...new Set(holidays)] });
      });
      await deps.syncBots();
      res.json({ ok: true });
    }));
    app.post("/api/teachers/person", wrap(async (req, res) => {
      ...
      await updateJSON(TEACHERS_PATH, (draft) => {
        if (getStudents()[`${num}@c.us`] || num === draft[TEACHERS_PATH].number) throw new Error("Nomor sudah dipakai siswa atau bot TU.");
