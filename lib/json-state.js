class JsonState {
  constructor({ writeBatch, initial = {} }) {
    this.writeBatch = writeBatch;
    this.cache = structuredClone(initial);
    this.tail = Promise.resolve();
  }

  replace(initial) {
    this.cache = structuredClone(initial);
  }

  read(key, fallback = {}) {
    return structuredClone(this.cache[key] ?? fallback);
  }

  readSelected(key, select, fallback = {}) {
    return structuredClone(select(this.cache[key] ?? fallback));
  }

  update(keys, mutate) {
    return this.transact(keys, mutate, (draft) => this.writeBatch(draft));
  }

  transact(keys, mutate, commit) {
    const uniqueKeys = [...new Set(Array.isArray(keys) ? keys : [keys])];
    const operation = this.tail.then(async () => {
      const draft = Object.fromEntries(
        uniqueKeys.map((key) => [key, this.read(key)])
      );
      const result = await mutate(draft);

      // Publish the new cache only after every durable write succeeds.
      await commit(draft, result);
      for (const key of uniqueKeys) this.cache[key] = structuredClone(draft[key]);
      return result;
    });

    this.tail = operation.catch(() => {});
    return operation;
  }

  exclusive(operation) {
    const result = this.tail.then(operation);
    this.tail = result.catch(() => {});
    return result;
  }
}

module.exports = { JsonState };
